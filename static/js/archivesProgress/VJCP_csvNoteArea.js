...
let csvNoteArea = Vue.component('csv-note-area', {
  template: `<div>
      ...
      <card-sec-searched>
        ...
        <template #contents>
          ...
          <div align="right" style="margin: 10px 5px; align-items: center;">
            ...
            <v-btn v-if="viewTypeToggle==0" :style="colorPalette.greenBack" @click="noAppearingConfirmDialog = true">全文</v-btn>
    ...
    <dialog-frame-normal :target="noAppearingConfirmDialog" :title="'ノート選択不能化確認'" :contents="'現在選択されているノートから表示を切り替えられなくなります。よろしいですか？（再検索することで解除されます）'">
      <v-btn :style="colorPalette.brownFront" @click="changeFornoAppearingNotes()">実行</v-btn>
      <v-btn @click="noAppearingConfirmDialog = false" :style="colorPalette.brownBack">キャンセル</v-btn>
    </dialog-frame-normal>
