...
<body>
  ...
  <div id="vueForCommon">
    ...
    <div id="wrapper">
      ...
      <section id="main">
        <!-- メッセージエリア -->
        <div id="messageArea" v-if="openSection.message" :class="openSection.message==true ? 'fader' : 'none'">
          <slogan>
            <template #sub>
        ...
        <!-- 検索フォームエリア -->
        <div id="searchFormArea" v-if="openSection.search" :class="openSection.search==true ? 'fader' : 'none'">
          <card-sec>
            <template #title><tag-title>検索フォーム</tag-title></template>
            <template #contents>
              <search-inner :prop="client">
                <div v-if="client.form.search.which.noteOrVideo==1 && client.form.search.gawty!=''" :class="client.form.search.which.noteOrVideo==1 ? 'fader' : 'none'">
              ...
              <div align="center">
                <p v-if="createdDoubleSet == true" style="color:red;cursor:pointer;" @click="createdDoubleSet = false" v-text="'自登録のみを表示する場合は、登録者名を入力しないでください'"></p>
                ...
                <div id="setlogInsert" v-if="client.form.search.gawty!=''" style="margin-bottom:10px;" :class="client.form.search.gawty!='' ? 'fader' : 'none'">
        ...
        <!-- GAWTYボタンエリア -->
        <div v-if="openSection.gawty" id="gawtyButtonArea" :class="openSection.gawty==true ? 'fader' : 'none'">
          <card-sec-searched :prop="'gawty'">
            <template #title><tag-title>GAWTYリンクボタン</tag-title></template>
            <template #contents>
              <div id="gawtyAreaInner" align="center">
                <v-btn :style="client.palette.blueFront + styles.mg5 + styles.ntf" target="_blank" :href="gawtyItems.google" v-text="'Google'"></v-btn>
                <v-btn :style="client.palette.greenFront + styles.mg5 + styles.ntf" target="_blank" :href="gawtyItems.allabout" v-text="'All About'"></v-btn>
                <v-btn :style="client.palette.yellowFront + styles.mg5 + styles.ntf" target="_blank" :href="gawtyItems.wikipedia" v-text="'Wikipedia'"></v-btn>
                <v-btn :style="'background-color:#93e6f0; color:#fff;' + styles.mg5 + styles.ntf" target="_blank" :href="gawtyItems.twitter" v-text="'Twitter'"></v-btn>
                <v-btn :style="client.palette.redFront + styles.mg5 + styles.ntf" target="_blank" :href="gawtyItems.youtube" v-text="'Youtube'"></v-btn>
              </div>
            </template>
          </card-sec-searched>
        </div>
